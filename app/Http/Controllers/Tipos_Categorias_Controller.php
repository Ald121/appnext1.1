...
class Tipos_Categorias_Controller extends Controller
{
    ...
    {
    $datos=
    DB::connection($this->name_bdd)->table('inventario.tipos_categorias')->insert(['nombre' => $request->name , 'descripcion' => $request->descripcion , 'estado' => 'A', 'fecha' => Carbon::now()->toDateString()]);
    return response()->json(['respuesta' => true], 200);
    }
    ...
    if ($request->has('filter')&&$request->filter!='') {
        ...
        $data=DB::connection($this->name_bdd)->table('inventario.tipos_categorias')->where('estado','A')->orderBy('nombre','ASC')->get();
    }

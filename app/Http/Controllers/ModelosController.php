...
class ModelosController extends Controller
{
    ...
    public function Existencia_Modelo(Request $request)
    {
        $existencia=DB::connection($this->name_bdd)->table('inventario.modelos')->where('nombre',$request->nombre)->get();
    ...
    if ($request->has('filter')&&$request->filter!='') {
        ...
        $data=DB::connection($this->name_bdd)->table('inventario.modelos')->where('estado','A')
                                                ->where('nombre','LIKE','%'.$request->input('filter').'%')
                                                //->orwhere('descripcion','LIKE','%'.$request->input('filter').'%')
                                                ->orderBy('nombre','ASC')->get();
    }else{
        $data=DB::connection($this->name_bdd)->table('inventario.modelos')->where('estado','A')->orderBy('nombre','ASC')->get();
    }
    ...
    {
    $data=DB::connection($this->name_bdd)->table('inventario.modelos')->where('id',$request->id)->update(['nombre' => $request->nombre , 'descripcion' => $request->descripcion]);
    return response()->json(['respuesta' => true], 200);
    }

    public function Delete_Modelo(Request $request)
    {
    $data=DB::connection($this->name_bdd)->table('inventario.modelos')->where('id',$request->id)->update(['estado'=>'I']);
    return response()->json(['respuesta' => true], 200);
    }
}
